...
def test_parametrized_route(api, client):
    ...
    assert client.get("http://testserver/Byron").text == "Hey, Byron!"
    assert client.get("http://testserver/David").text == "Hey, David!"
...
def test_template(api, client):
    ...
    def html_handler(req, resp):
        resp.body = api.template("index.html", context={"title": "Some title", "name": "Byron"}).encode()
...
def test_custon_exception_handler(api, client):
    def on_exception(req, resp, exc):
        resp.text = "AttributeErrorHappened"

    api.add_exception_handler(on_exception) # Pass the exception to the handler.
